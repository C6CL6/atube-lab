...
import { canBeat, identifyCombination, playCards, startRound, type Card, type RoundState, type Team } from '../../src/guandan/domain/rules'
...
function getStoreInstance() {
  return getStore({ name: 'guandan-rooms', consistency: 'strong' })
}
...
async function loadRoom(code: string) {
  const room = await getStoreInstance().get(roomKey(code), { type: 'json' }) as Room | null
  return room
}

async function saveRoom(room: Room) {
  await getStoreInstance().setJSON(roomKey(room.code), room)
}
...
function assertFresh(room: Room, expectedVersion: unknown) {
  if (typeof expectedVersion !== 'number' || expectedVersion !== room.version) {
    throw new Response(JSON.stringify({ error: '房间状态已更新，请刷新后重试', version: room.version }), {
      status: 409,
      headers: headers(),
    })
  }
...
function applyAction(room: Room, body: ActionBody): Room {
  ...
  if (!body.playerId) throw new Response(JSON.stringify({ error: '缺少玩家身份' }), { status: 400, headers: headers() })
  ...
  if (body.type === 'addBot') {
    if (next.status !== 'waiting') throw new Response(JSON.stringify({ error: '牌局已开始，不能补机器' }), { status: 400, headers: headers() })
    const host = next.seats.find((seat) => seat.seatIndex === 0)
    if (host?.playerId !== body.playerId) throw new Response(JSON.stringify({ error: '只有房主可以补机器' }), { status: 403, headers: headers() })
    const seatIndex = body.seatIndex
    if (seatIndex !== 1 && seatIndex !== 2 && seatIndex !== 3) throw new Response(JSON.stringify({ error: '请选择可补位的机器座位' }), { status: 400, headers: headers() })
    if (next.seats.some((seat) => seat.seatIndex === seatIndex)) throw new Response(JSON.stringify({ error: '该座位已经有人' }), { status: 400, headers: headers() })
  ...
  if (body.type === 'start') {
    if (next.seats.length !== 4 || next.seats.some((seat) => !seat.ready)) {
      throw new Response(JSON.stringify({ error: '需要四名玩家全部准备' }), { status: 400, headers: headers() })
    }
  ...
  if (body.type === 'play') {
    if (!next.round) throw new Response(JSON.stringify({ error: '牌局尚未开始' }), { status: 400, headers: headers() })
  ...
  throw new Response(JSON.stringify({ error: '未知动作' }), { status: 400, headers: headers() })
}

async function createRoom(req: Request) {
  const body = await req.json().catch(() => ({})) as ActionBody
  if (!body.playerId) return json({ error: '缺少玩家身份' }, 400)
  const now = new Date()
...
export default async (req: Request) => {
  try {
    ...
    if (req.method === 'POST' && url.pathname === '/api/guandan/rooms') return createRoom(req)
    ...
    return json({ error: '掼蛋房间服务暂不可用' }, 500)
  }
}

export const config: Config = {
  path: ['/api/guandan/rooms', '/api/guandan/rooms/:roomCode', '/api/guandan/rooms/:roomCode/actions'],
}
