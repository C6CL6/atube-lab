...
begin
  select payment.* into v_order
  from public.vpn_payment_orders as payment
  where payment.id = p_order_id
  for update;
  ...
  if exists (
    select 1 from public.vpn_payment_orders as other_order
    where other_order.alipay_trade_no = p_alipay_trade_no and other_order.id <> p_order_id
  ) then
    raise exception 'Alipay trade already claimed by another order' using errcode = '23505';
  end if;

  insert into public.vpn_machine_trial_ledger as ledger (machine_code_hash)
  values (v_order.machine_code_hash)
  on conflict on constraint vpn_machine_trial_ledger_pkey do update
    set updated_at = now()
  returning ledger.* into v_ledger;

  select ledger.* into v_ledger
  from public.vpn_machine_trial_ledger as ledger
  where ledger.machine_code_hash = v_order.machine_code_hash
  for update;
  ...
  if v_order.license_id is null then
    ...
    if v_trial_bonus_applied then
      v_duration_days := v_duration_days + least(15, greatest(0, p_trial_bonus_days));
    end if;

    update public.vpn_machine_trial_ledger as ledger
    set trial_bonus_consumed = ledger.trial_bonus_consumed or v_trial_bonus_applied,
        latest_license_expires_at = date_trunc('day', v_base) + make_interval(days => v_duration_days) - interval '1 second',
        first_issued_at = coalesce(ledger.first_issued_at, now()),
        updated_at = now()
    where ledger.machine_code_hash = v_order.machine_code_hash
    returning ledger.* into v_ledger;

    update public.vpn_payment_orders as payment
    set status = 'paid',
    ...
    where payment.id = p_order_id
    returning payment.* into v_order;
  else
    update public.vpn_payment_orders as payment
    set status = case when payment.status = 'licensed' then 'licensed' else 'paid' end,
        alipay_trade_no = p_alipay_trade_no,
        paid_at = coalesce(payment.paid_at, now())
    where payment.id = p_order_id
    returning payment.* into v_order;
...
begin
  ...
  update public.vpn_payment_orders as payment
  set status = 'licensed',
      license_key = coalesce(payment.license_key, p_license_key),
      licensed_at = coalesce(payment.licensed_at, now())
  where payment.id = p_order_id
  returning payment.* into v_order;
